...
#                 - $score         : 품질 점수 (0~100 정수)
#                 - $missing       : data.frame(variable, missing_count, missing_percent)
#                 - $outliers      : data.frame(variable, outlier_count, outlier_percent)
#                 - $format        : data.frame(variable, unique_values, issue)
...
generate_html_report <- function(file, data, report, suitability,
  ...
  n_date    <- sum(sapply(data, function(x) inherits(x, "Date")))
  ...
  # 결측치 위험 컬럼
  if (!is.null(report$missing) && nrow(report$missing) > 0) {
    danger_cols <- report$missing$variable[report$missing$missing_percent >= 10]
    warn_cols   <- report$missing$variable[report$missing$missing_percent > 0 &
                                             report$missing$missing_percent < 10]
    
    if (length(danger_cols) > 0) {
      findings_html <- paste0(findings_html, "
        <div style='background:#f8d7da; border-left:4px solid #dc3545;
                    padding:12px 16px; margin:10px 0; border-radius:4px; color:#721c24;'>
          ⚠️ <strong>즉시 조치 필요 (결측률 10% 이상):</strong> ",
                              esc(paste(danger_cols, collapse = ", ")),
                              " — 원본 데이터를 재확인하거나 대체값 처리 전략을 수립하세요.
        </div>")
    }
    if (length(warn_cols) > 0) {
      findings_html <- paste0(findings_html, "
        <div style='background:#fff3cd; border-left:4px solid #ffc107;
          ...
          🔔 <strong>검토 권장 (결측치 존재):</strong> ",
                              esc(paste(warn_cols, collapse = ", ")),
                              " — 분석 전 결측치 처리를 권장합니다.
        </div>")
    }
  }
  ...
  if (n_dup_rows > 0) {
    findings_html <- paste0(findings_html, "
      <div style='background:#fff3cd; border-left:4px solid #ffc107;
        ...
        중복되는 행이 존재합니다. 분석 목적에 따라 중복 제거 여부를 결정하세요.
      </div>")
  }
  
  # 이상치
  if (!is.null(report$outliers) && nrow(report$outliers) > 0) {
    out_cols <- report$outliers$variable
    findings_html <- paste0(findings_html, "
      <div style='background:#fff3cd; border-left:4px solid #ffc107;
                  padding:12px 16px; margin:10px 0; border-radius:4px; color:#856404;'>
        🔔 <strong>이상치 발견:</strong> ",
                            esc(paste(out_cols, collapse = ", ")),
                            " — IQR 기준 이상치가 존재합니다. 분석 목적에 맞게 처리 여부를 결정하세요.
      </div>")
  }
  
  # 형식 문제
  if (!is.null(report$format) && nrow(report$format) > 0) {
    fmt_cols <- report$format$variable
    findings_html <- paste0(findings_html, "
      <div style='background:#fff3cd; border-left:4px solid #ffc107;
                  padding:12px 16px; margin:10px 0; border-radius:4px; color:#856404;'>
        🔔 <strong>형식 일관성 문제:</strong> ",
                            esc(paste(fmt_cols, collapse = ", ")),
                            " — 대소문자 혼재 또는 고유값 비율이 높습니다.
      </div>")
  }
  ...
  <div class="body-content">
    ...
    <div class="section">
      ...
      <p><strong>🔹 결측치</strong></p>',
                 ...
                 if (!is.null(report$missing) && nrow(report$missing) > 0) {
                   rows <- paste0(sapply(seq_len(nrow(report$missing)), function(i) {
                     r <- report$missing[i, ]
      ...
      <p><strong>🔹 이상치 (IQR 기준, 수치형 변수)</strong></p>',
                 ...
                 if (!is.null(report$outliers) && nrow(report$outliers) > 0) {
                   rows <- paste0(sapply(seq_len(nrow(report$outliers)), function(i) {
                     r <- report$outliers[i, ]
                     paste0("<tr><td><strong>", esc(r$variable), "</strong></td>",
      ...
      <p><strong>🔹 형식 일관성 (문자형 변수)</strong></p>',
                 ...
                 if (!is.null(report$format) && nrow(report$format) > 0) {
                   rows <- paste0(sapply(seq_len(nrow(report$format)), function(i) {
                     r <- report$format[i, ]
                     paste0("<tr><td><strong>", esc(r$variable), "</strong></td>",
                            "<td>", esc(r$issue), "</td>",
                            "<td>", r$unique_values, "개</td></tr>")
                   }), collapse = "\n")
                   paste0("
      <div class='warn-box'>
        <div class='warn-box-title'> 형식 일관성 문제가 발견된 변수</div>
        <table>
          <thead><tr><th>변수명</th><th>문제 유형</th><th>고유값 수</th></tr></thead>
          <tbody>", rows, "</tbody>
        </table>
      </div>")
                 } else {
                   "<div class='ok-box'> 형식 일관성이 좋습니다.</div>"
                 },
